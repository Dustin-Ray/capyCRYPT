...
fn bench_sign_verify(c: &mut Criterion) {
    c.bench_function("3448 + SHA3-256 Sign + Verify Roundtrip", |b| {
        b.iter(|| {
            sign_verify(
                KeyPair::new(&get_random_bytes(16), "test string".to_string(), &BIT_SECURITY).unwrap(),
...
fn bench_key_gen_enc_dec(c: &mut Criterion) {
    c.bench_function("e448 + SHA3-256 Asymmetric enc + dec", |b| {
        b.iter(|| {
            key_gen_enc_dec(
                &mut KeyPair::new(&get_random_bytes(32), "test string".to_string(), &BIT_SECURITY)
                    .unwrap()
                    .priv_key,
                    Message::new(get_random_bytes(524880)),
            )
        });
    });
}
