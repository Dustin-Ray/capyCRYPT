...
pub mod model_test {
    ...
    pub fn test_signature() {
        ...
        // let mut message = get_random_bytes(5242880);
        let message = hex::decode("").unwrap();    
        let pw = "testtest".as_bytes().to_vec();
        let key_obj = gen_keypair(&mut pw.clone(), "test".to_string());
        let x = key_obj.pub_x;
        let y = key_obj.pub_y;
        let mut key = get_e521_point(x, y);

        let sig = sign_with_key(&mut pw.clone(), &mut message.clone());
        // println!("{:?}", sig);
        let res = verify_signature(&sig, &mut key, &mut message.clone());
