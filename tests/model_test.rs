#[cfg(test)]
pub mod model_test {
    use std::{time::Instant, borrow::BorrowMut};
    use cryptotool::{
        model::shake_functions::{encrypt_with_pw, decrypt_with_pw, gen_keypair, encrypt_with_key, decrypt_with_key, sign_with_key, verify_signature}, 
        curve::e521::e521_module::{get_e521_point}};
    use cryptotool::sha3::aux_functions::byte_utils::get_random_bytes;
    
    use std::cell::RefCell;

    #[test]
    pub fn test_sym_enc() {
        let mut total = 0.0;
        let rounds = 1.0;
        for _ in 0..rounds as i32 {
            let pw = get_random_bytes(16);
            let mut message = RefCell::new(get_random_bytes(5242880).to_owned());
            // let mut message = hex::decode("").unwrap();

            let now = Instant::now();
            let mut cg2 = encrypt_with_pw(&mut pw.clone(), &mut message);
            let res = decrypt_with_pw(&mut pw.clone(), &mut cg2);
            // println!("{:?}", cg2.c.clone());
            let elapsed = now.elapsed();
            let sec = (elapsed.as_secs() as f64) + (elapsed.subsec_nanos() as f64 / 1000_000_000.0);
            total += sec;
            assert!(res);
        }
        println!("Code took: {} seconds", total / rounds);
    }

    #[test]
    fn test_key_gen_enc_dec() { //check conversion to and from bytes.

        let mut total = 0.0;
        let rounds = 10.0;
        for _ in 0..rounds as i32 {
            let pw = get_random_bytes(16);
            let owner = "test key".to_string();
            let mut message = get_random_bytes(52428);
            // let mut message = hex::decode("").unwrap();    

            let key_obj = gen_keypair(&mut pw.clone(), owner);
            let x = key_obj.pub_x;
            let y = key_obj.pub_y;
            let mut pub_key = get_e521_point(x, y);
            let now = Instant::now();
            let enc = encrypt_with_key(&mut pub_key, &mut message);
            // println!("enc{:?}: ", hex::encode(enc.c.clone()));
            let res = decrypt_with_key(&mut pw.clone(), enc);
            // println!("dec{:?}: ", hex::encode(enc.c.clone()));
            let elapsed = now.elapsed();
            let sec = (elapsed.as_secs() as f64) + (elapsed.subsec_nanos() as f64 / 1000_000_000.0);
            total += sec;
            assert!(res);
        }
        println!("Code took: {} seconds", total / rounds);
    }

    #[test]
    pub fn test_signature() {

        // let mut message = get_random_bytes(5242880);
        let message = RefCell::new(get_random_bytes(5242880).to_owned());        
        let pw = get_random_bytes(16);
        let key_obj = gen_keypair(&mut pw.clone(), "test".to_string());
        let x = key_obj.pub_x;
        let y = key_obj.pub_y;
        let mut key = get_e521_point(x, y);

        let sig = sign_with_key(&mut pw.clone(), &message);
        // println!("{:?}", sig);
        let res = verify_signature(&sig, &mut key, &message);
        
        println!("good sig?: {:?}", res);
    }

}