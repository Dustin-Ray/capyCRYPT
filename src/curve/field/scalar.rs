...
fn montgomery_multiply(
    ...
    for i in 0..x.as_limbs().len() {
        ...
        for (j, &ylimb) in y.as_limbs().iter().enumerate() {
            chain += u128::from(x.as_limbs()[i]) * u128::from(ylimb) + u128::from(result.as_limbs()[j]);
        ...
        for (j, &mlimb) in Modulus::MODULUS.as_limbs().iter().enumerate() {
            chain += (u128::from(multiplicand)) * u128::from(mlimb) + u128::from(result.as_limbs()[j]);
            if j > 0 {
...
#[test] fn test_add() {
    ...
    const R: U448 = Uint::MAX.const_rem(&Modulus::MODULUS).0.wrapping_add(&Uint::ONE);
