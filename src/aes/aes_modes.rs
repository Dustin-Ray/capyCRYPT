...
pub fn encrypt_aes_ecb(input: &Vec<u8>, output: &mut Vec<u8>, key: &str) {
    let key_schedule = AES::new(key);
    let round_key = &key_schedule.round_key;

    for block_index in (0..input.len()).step_by(16) {
        AES::encrypt_block(&input, output, block_index, &round_key);
    }
}

pub fn decrypt_aes_ecb(input: &Vec<u8>, output: &mut Vec<u8>, key: &str) {
    let key_schedule = AES::new(key);
    let round_key = &key_schedule.round_key;

    for block_index in (0..input.len()).step_by(16) {
        AES::decrypt_block(&input, output, block_index, &round_key);
    }
}
